...
describe('Task Stream Container', () => {
  ...
  it('should fail to queue: interval timings, not enough time sequentially', async () => {
    ...
    expect(s1.handler).toHaveBeenCalledWith(0)
    expect(s1.handler).toHaveBeenCalledWith(s1.interval)
    // since s1 was still in progress wait 1000
    expect(s2.handler).toHaveBeenCalledWith(1000)
    expect(s2.handler).toHaveBeenCalledWith(s1.interval)
  })
  ...
  it('should fail to queue: interval timings, not enough time extended II', async () => {
    ...
    const s54 = streams.create('id54', due, repetitions, 5000, spy())
    //const s7 = streams.create('id7', due, 2, 7000, spy())

    const all = [s51, s52, s53, s54].map((s) => streams.add(s))

    expect(streams.create.bind(streams, 'id7', Date.now(), 3, 7000)).toThrowError(
      'Stream collision [id7, 7000ms], spacing is 1000 for 4x5000'
    )
    ...
    //await streams.add(streams.create('id55', due, repetitions, 4999, spy()))

    await Promise.all(all)
    await streams.run()

    /* without the guard this queues into collision in the second iteration
